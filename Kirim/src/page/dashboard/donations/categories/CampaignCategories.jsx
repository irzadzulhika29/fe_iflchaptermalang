...
import {
  useAddCategoryCampaignByAdmin,
  useDeleteCategoryCampaignByAdmin,
  useEditCategoryCampaignByAdmin,
  useGetAllCategoriesCampaign,
} from "../../../../features/campaign/services";

import { Funnel } from "@phosphor-icons/react";

import AddCampaignCategoryModal from "./AddCampaignCategoryModal";
import EditCampaignCategoryModal from "./EditCampaignCategoryModal";
import DeleteCampaignCategoryModal from "./DeleteCampaignCategoryModal";
              ...
              <tr key={index} className="border-b bg-light-1">
                ...
                <td className="px-6 py-4">
                  <div className="flex overflow-hidden border border-gray-500 divide-x-2 w-max rounded-xl">
                    <EditCampaignCategoryModal id={item?.id} data={item} editCategory={editCategory} editPending={editPending} />
                    <DeleteCampaignCategoryModal id={item?.id} deleteCategory={deleteCategory} deletePending={deletePending} />
                  </div>
                </td>
              </tr>
            ))}
...
const CampaignCategories = () => {
  ...
  const { data: categories, isLoading: categoriesLoading } = useGetAllCategoriesCampaign();

  const { mutate: addCategory, isPending: addPending } = useAddCategoryCampaignByAdmin();

  const { mutate: editCategory, isPending: editPending } = useEditCategoryCampaignByAdmin();

  const { mutate: deleteCategory, isPending: deletePending } = useDeleteCategoryCampaignByAdmin();
  ...
  return (
    <Dashboard title="Campaign Categories" detail={detail}>
      <div className="flex flex-col items-center justify-between gap-4 my-4 sm:flex-row">
        ...
        <Button onClick={() => setShowModal(true)} ariaLabel="navigate-add-category" intent="secondary">
          Add Category Campaign
        </Button>
        <AddCampaignCategoryModal showModal={showModal} setShowModal={setShowModal} addPending={addPending} addCategory={addCategory} />
      </div>
      {categoriesLoading ? (
